...
const dummyPriceData = [
  { time: '00:00', price: 0.15 },
  { time: '01:00', price: 0.12 },
  { time: '02:00', price: 0.10 },
  { time: '03:00', price: 0.11 },
  { time: '04:00', price: 0.13 },
  { time: '05:00', price: 0.18 },
  { time: '06:00', price: 0.22 },
  { time: '07:00', price: 0.28 },
  { time: '08:00', price: 0.35 },
  { time: '09:00', price: 0.40 },
  { time: '10:00', price: 0.38 },
  { time: '11:00', price: 0.35 },
  { time: '12:00', price: 0.32 },
  { time: '13:00', price: 0.30 },
  { time: '14:00', price: 0.28 },
  { time: '15:00', price: 0.25 },
  { time: '16:00', price: 0.27 },
  { time: '17:00', price: 0.32 },
  { time: '18:00', price: 0.38 },
  { time: '19:00', price: 0.35 },
  { time: '20:00', price: 0.30 },
  { time: '21:00', price: 0.25 },
  { time: '22:00', price: 0.20 },
  { time: '23:00', price: 0.18 },
];
...
const ElectricityManagement = () => {
  ...
  const [solarBatteryLevel, setSolarBatteryLevel] = useState(
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      const currentIsHigh = priceData.price > 0.25;
      const nextIsHigh = nextPrice.price > 0.25;
      
      if (currentIsHigh !== nextIsHigh) {
        setNotification({
          type: nextIsHigh ? 'warning' : 'success',
          title: nextIsHigh ? 'High Price Period Approaching' : 'Low Price Period Approaching',
          message: nextIsHigh 
            ? `Electricity prices will increase to $${nextPrice.price.toFixed(2)}/kWh at ${nextPrice.time}`
            : `Electricity prices will decrease to $${nextPrice.price.toFixed(2)}/kWh at ${nextPrice.time}`
        });
      }
      ...
      if (currentIsHigh && now.getSeconds() % 3 === 0) {
        setSolarBatteryLevel(prev => Math.max(0, prev - 0.2));
      }
    }, 1000);
        ...
        <div className="p-6">
          <div className="text-center mb-6">
            ...
            <div className="text-xl">
              Current Price: <span className={`font-bold ${isHighPriceState ? 'text-red-600' : 'text-green-600'}`}>
          ...
          {notification && (
            <div className={`mb-6 p-4 rounded-lg flex items-start gap-3 ${
              notification.type === 'warning' ? 'bg-amber-50 text-amber-800' : 'bg-green-50 text-green-800'
            }`}>
          ...
          <div className={`rounded-lg p-4 mb-6 ${isHighPriceState ? 'bg-red-50' : 'bg-green-50'}`}>
            ...
            <div className="bg-gray-100 rounded-full h-6 overflow-hidden">
              <div 
                ...
                style={{
                  ...
                  backgroundColor: `${solarBatteryLevel > 50 ? '#22c55e' : solarBatteryLevel > 20 ? '#f59e0b' : '#ef4444'}`
                }}
              />
            </div>
              ...
              {equipment.map((item, index) => (
                ...
                >
                  ...
                  <div className={`text-sm ${
                    ...
                    {!isHighPriceState || item.canUseOnSolar ? 'Available' : 'Unavailable'}
                  </div>
                </div>
              ))}
            </div>
